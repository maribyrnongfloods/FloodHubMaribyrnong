...
- **Daily ERA5-Land forcings** (__PSTART__ → __PEND__), 14 variables aggregated from hourly UTC to local time.
...
- **Period**: the merged timeseries runs __PSTART__ → __PEND__, bounded by the ERA5-Land forcing export. Per-gauge streamflow periods are recorded in `attributes_additional`.
