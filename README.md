...
- **Daily streamflow** (mm/day) per gauge, area-normalised with the GEE Part-1 polygon areas. The Keilor mainstem record (`ausvic_230200`) extends back to **1908-02-02**; rows before 1950-01-02 carry streamflow only, with empty forcing columns — the Caravan convention for records that pre-date ERA5-Land.
- **Daily ERA5-Land forcings** from 1950-01-02 to the bundle end date (per-gauge periods in `attributes_additional`): 14 ERA5-Land variables aggregated from hourly to daily (mean/min/max where applicable, plus a precipitation total and two potential-evaporation estimates).
- **Attributes**: `attributes_other` (the 6 standard Caravan columns, in the standard order, country name `Australia`), `attributes_hydroatlas` (gauge_id + 196 BasinATLAS attributes from the official Part-1 notebook), 14 derived climate indices (`attributes_caravan`), and an extension-specific `attributes_additional` (basin name, streamflow period/missing/units, source, license, notes).
- **Single combined shapefile** `ausvic_basin_shapes.shp` (gauge_id only, EPSG:4326), 10 distinct simplified polygons.
- **Licenses**: `licenses/ausvic/license_ausvic.md` (full attributions) plus a root `LICENSE.txt` pointer.
...
Polygons are derived from the Melbourne Water Stream Network (MWSTR) by tracing each gauge's upstream sub-catchments; they are distinct and properly nested (e.g. Konagaderra ⊂ Bulla ⊂ Keilor North ⊂ Keilor ⊂ Chifley Drive; nested containment 99.9–100% by area). Each gauge sits at the **outlet** — the downstream tip — of its own catchment (6–300 m from the polygon edge, versus 6–38 km from the centroid), confirming correct gauge-to-polygon placement:

![Each gauge (red) sits at the downstream outlet of its catchment, far from the centroid (grey ×).](outlet_proof.png)

## Streamflow sources

Seven Melbourne Water gauges (230100, 230102, 230106, 230107, 230119, 230211, 230237) are taken from **BoM Water Data Online** — the quality-controlled "Water Course Discharge" series (`DMQaQc.Merged.AsStored`), aggregated from sub-daily to a time-weighted daily mean. This replaces the earlier Melbourne Water API source and is cleaner and longer (Bulla reaches back to 1955), with the earlier API's spurious flood spikes removed. The three Victorian gauges — 230200 (Keilor), 230202 (Sunbury), 230206 (Gisborne) — come from the Victorian Water Measurement Information System (Hydstra) daily-discharge API (variable 141.00); the Keilor mainstem record is the longest in the set, beginning in **1908** (streamflow-only rows before 1950, ~10,200 valid pre-1950 days).

## Flood record

The largest Maribyrnong floods are clearly resolved. At Keilor (mainstem, record from 1908) the three biggest daily-mean peaks on record are **14 October 2022 (33.25 mm/d ≈ 503 m³/s)**, **24 September 1916 (30.38 mm/d ≈ 459 m³/s)** and **16 May 1974 (25.25 mm/d ≈ 382 m³/s)** — matching the SES historic stage table, where 1916 (4.26 mAHD) and 2022 (4.22 mAHD) are the two largest post-1908 events. The 2022 event ranks first by flow but below 1916 by stage, consistent with a century of floodplain and rating changes. October 2022 is rain-driven (≈57 mm of ERA5-Land precipitation on 13 October) and builds downstream along Deep Creek and the mainstem — Konagaderra ≈214 → Bulla ≈317 → Keilor ≈503 m³/s daily mean, with Chifley Drive ≈494 (slightly damped by tidal backwater). The May 1974 flood appears at the four gauges whose records predate it (Keilor, Bulla, Sunbury, Gisborne).
...
- **230237 (Keilor North): Oct-2022 peak reads low.** The daily-mean peak (~313 m³/s) is below both upstream (Bulla + Sunbury ≈364 m³/s combined) and downstream (Keilor ≈503 m³/s, corroborated by the Jacobs 2023 post-event analysis) — likely a high-stage rating underestimate. Treat major-flood peaks at this gauge with caution. See `attributes_additional`.
...
- **Period**: ERA5-Land forcings run from 1950-01-02 to the export date; the Keilor streamflow record additionally covers 1908–1949 as streamflow-only rows with empty forcing columns. Per-gauge streamflow periods are recorded in `attributes_additional`.

## Files

- `attributes/ausvic/` — `attributes_other_ausvic.csv` (6 standard cols, standard order), `attributes_hydroatlas_ausvic.csv` (gauge_id + 196 attrs), `attributes_caravan_ausvic.csv` (14 climate indices), `attributes_additional_ausvic.csv` (extension-specific)
...
- `LICENSE.txt` — root license pointer
