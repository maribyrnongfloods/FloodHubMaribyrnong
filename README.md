...
- **Attributes**: `attributes_other` (the 6 standard Caravan columns, in the standard order, ISO country code `AU`), `attributes_hydroatlas` (gauge_id + 196 BasinATLAS attributes from the official Part-1 notebook), 14 derived climate indices (`attributes_caravan`), and an extension-specific `attributes_additional` (basin name, streamflow period/missing/units, source, license, notes).
